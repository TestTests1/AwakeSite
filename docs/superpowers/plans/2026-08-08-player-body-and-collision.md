...
- [ ] **Шаг 2: считать отход камеры от тела, а не от глаз**

В `Player.tsx` в `finish()` луч отхода камеры пускается из `camera.position`.
Она стоит ровно на точке глаз, а та при вплотную подошедшем игроке может
оказаться уже внутри блока — тогда луч стартует изнутри стены и не находит
ничего. Считать от центра тела и вычитать из найденного расстояния запас:

```ts
      if (thirdPerson) {
        // Луч отхода пускается от середины тела, а не от глаз: глаза стоят на
        // самой макушке, и у стены эта точка бывает уже внутри блока — луч из
        // такого начала ничего не находит, и камера уезжает сквозь стену.
        camera.getWorldDirection(forward.current)
        back.current.copy(forward.current).negate()
        origin.current.set(
          feet.current.x,
          feet.current.y + bodyHeight.current / 2,
          feet.current.z,
        )
        let distance = CAMERA_BACK
        const hit = collider.cast(origin.current, back.current, CAMERA_BACK + CAMERA_MARGIN)
        if (hit !== null) distance = Math.max(CAMERA_MIN, hit - CAMERA_MARGIN)
        camera.position.copy(origin.current).addScaledVector(back.current, distance)
        camera.position.y = feet.current.y + EYE_HEIGHT
      }
```

**Порядок задач:** `bodyHeight` заводится в задаче 5. Если задача 3 делается
раньше, взять здесь постоянную высоту стоя и поправить в задаче 5 — это
отмечено в её шаге 3.
