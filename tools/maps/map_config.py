...
# Точки появления, выбранные руками.
#
# По умолчанию compute_spawn.py ставит игрока в середину карты над её крышей,
# чтобы он упал на первую попавшуюся поверхность. Для целой модели это годилось,
# для подгрузки кусками — нет: при входе карта ещё не приехала, падать не на что,
# и точка над крышей означает висение в небе, пока едут куски. Да и середина
# карты — случайное место, а заходят люди работать с заграждениями.
#
# Координаты сняты в игре и проверены по кэшу карты: под точкой сплошной столб,
# над ней два свободных блока под рост игрока.
SPAWN_OVERRIDES: Dict[str, Tuple[float, float, float]] = {
    "hvoiny": (3865, 69, -2075),
}
