  ...
  useFrame((_, rawDelta) => {
    ...
    if (toGround !== null) {
      ...
      if (!bounds.isEmpty()) {
        ...
        // отсутствует, но координаты снаружи footprint карты — там кускам
        // взяться неоткуда и никогда не будет, это не «не прилетело», а
        // «здесь и нет ничего»: тоже падаем и полагаемся на страховку ниже,
        // а не замираем без единого способа из этого выйти. Замираем только
        // если координаты внутри карты и кусок правда ещё не прилетел.
        const insideMap =
        ...
        if (insideMap && !collider.coversColumn(feet.current.x, feet.current.z)) {
