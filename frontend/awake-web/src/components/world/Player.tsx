...
/**
 * Сколько секунд ждать землю под точкой появления, прежде чем сдаться и
 * включить гравитацию.
 *
 * Предел нужен на случай, когда земли там не будет никогда: точка появления
 * пришлась на дырку в разрежённой сетке или кусок не приехал совсем. Висеть
 * вечно хуже, чем упасть, — падение хотя бы видно и лечится возвратом на точку
 * появления, который уже есть ниже по кадру.
 */
const WAIT_FOR_GROUND = 20
...
export function Player({
  ...
  /**
   * Нашлась ли опора под точкой появления.
   *
   * Точка появления из экспортёра стоит над крышей мира нарочно: с целой
   * моделью карта уже вся на месте, поиск опоры при входе срабатывает сразу, и
   * игрок встаёт на землю, ни разу не падая. С подгрузкой кусками при входе не
   * загружено ещё ничего — искать опору не в чем, и та же точка означает
   * падение с сотни метров сквозь пустоту, пока куски едут.
   *
   * Поэтому до первой найденной опоры игрок висит на месте, а не падает: как
   * только под ним появляется земля, он на неё встаёт.
   */
  const settled = useRef(false)
  /** Сколько уже ждём опору. Нужен предел — см. WAIT_FOR_GROUND. */
  ...
  const placeAtSpawn = useCallback(() => {
    ...
    // С целой моделью опора находится сразу и ждать нечего; с кусками — нет, и
    // тогда кадровый цикл подержит игрока на месте, пока земля не приедет.
    settled.current = found !== null
    waiting.current = 0
  ...
  useFrame((_, rawDelta) => {
    ...
    // Ждём землю, а не падаем сквозь пустоту: при входе куски ещё едут, и
    // опоры под точкой появления просто нет. Один луч в кадр и только до
    // первой удачи — как только земля нашлась, встаём на неё и больше сюда не
    // заходим.
    if (!settled.current) {
      const ground = collider.groundAt(feet.current.x, feet.current.z, bounds)
      if (ground !== null) {
        ...
        if (waiting.current < WAIT_FOR_GROUND) {
          // висим неподвижно: ходить по несуществующей карте всё равно некуда
          velocityY.current = 0
          finish(false)
          return
        }
