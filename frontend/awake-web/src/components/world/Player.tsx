...
export function Player({
  ...
  useFrame((_, rawDelta) => {
    ...
    if (toGround !== null) {
      ...
      // точно, а высота — нет. Кусок есть, а опоры нет — значит под ним
      // настоящая открытая пустота, и падаем, как раньше этой задачи. Куска
      // нет вовсе — значит он не прилетел, и замираем на месте: уронить
      // игрока сквозь незагруженный пол хуже, чем задержать на миг.
      if (!collider.coversColumn(feet.current.x, feet.current.z)) {
